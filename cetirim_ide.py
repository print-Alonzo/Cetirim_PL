...
class CetirimIDE(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for text, callback in (("New", self.new_file), ("Open", self.open_file), ("Save", self.save_file),
                               ("Run ▶", self.run_program), ("Check", self.check_code),
        ...
        # Keep requested height independent of the editor font.  The parent
        # pane expands to fill spare space, leaving the terminal usable.
        self.line_numbers = tk.Text(center, width=4, height=1, padx=6, takefocus=0, state="disabled", background="#1d2330", foreground="#8792a2", borderwidth=0, font=("Consolas", 13), cursor="arrow")
        ...
        console_frame, problems_frame = ttk.Frame(bottom), ttk.Frame(bottom)
        bottom.add(console_frame, text=" Output "); bottom.add(problems_frame, text=" Problems ")
    ...
    def _make_menu(self):
        ...
        for label, callback, key in (("Run", self.run_program, "F5"), ("Check syntax", self.check_code, ""), ("Rename symbol", self.rename_symbol, "Ctrl+R"), ("Insert template", self.show_templates, "Ctrl+Space")):
