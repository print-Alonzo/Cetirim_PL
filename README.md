...
- **Step In** (`F8`) — run just the next source line, then pause again, descending into any function that line calls.
- **Step Over** (`F10`) — the same, except a function call on that line runs to completion instead of pausing inside it, so you trace the current function without walking through every callee. A breakpoint inside the skipped function still stops execution.
